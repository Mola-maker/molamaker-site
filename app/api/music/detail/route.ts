...
// GET /api/music/detail?id=<netease-song-id>
// Returns full song metadata (cover, lyrics, album, duration) for any song ID.
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const album   = (detail.al ?? {}) as Record<string, unknown>;
    const artists = ((detail.ar ?? []) as Record<string, unknown>[])
    ...
    return NextResponse.json({
      data: {
        id,
        title:    String(detail.name ?? 'Unknown'),
        artist:   artists || 'Unknown artist',
        album:    String(album.name ?? ''),
        cover:    String(album.picUrl ?? ''),
        duration: Math.floor(Number(detail.dt ?? 0) / 1000),
        lyrics,
      },
    });
  } catch (err) {
